...
| 3.4 | Tap **"Sync Now"** | Spinner appears with progress text: "Uploading 1 of 3..." | ✅ |
...
| 8.4 | Tap "Sync Now" | Progress shows "Uploading 1 of 12..." and continues one by one | ✅ |
...
## 🛠️ Phase 4 Troubleshooting & Bug Log

| Issue Encountered | Root Cause | Resolution |
|-------------------|------------|------------|
| **Empty "details" in Firestore** | Server saved `details: ""` for scans with no notes. | Added a check in `scan.controller.js` to only include the field if it's non-empty. |
| **Sync Modal never appeared** | `NetInfo` monitoring wasn't started. | Added `useEffect` with `startMonitoring()` in `app/_layout.tsx`. |
| **Sync Modal popped up on app start while offline** | Default state of network store was `isOnline: true`. | Changed `useNetworkSync` hook to perform an async `NetInfo.fetch()` on mount. |
| **"Network Error" on Sync Now** | USB debugging required port 3000 to be reversed. | Ran `adb reverse tcp:3000 tcp:3000` to tunnel backend traffic. |
| **Sync Modal unmounted instantly** | Marking items as `isSyncing` removed them from the `pendingItems` filter. | Refactored `SyncPromptModal` to use a separate count for unmount conditions. |
| **User locked in Modal during sync** | Modal was blocking and didn't allow app exploration. | **UX Refactor:** Moved sync logic to `useSyncStore.runSync()` and added a global `SyncStatusBanner`. |
| **Interruption test (7.3) failed** | Airplane mode doesn't kill USB `adb reverse` tunnels. | Changed simulation to use `adb reverse --remove tcp:3000`. |
